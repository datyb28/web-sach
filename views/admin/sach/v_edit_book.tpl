...
    <li>Trang tổng quan</li>
	<li>Quản lý sách</li>
	<li>Cập nhật sách</li>
	<li class="active">{$hienthithongtin->p_name}</li>
</ol>
<div class="row">
		...
		<div class="panel-body">
			...
			<div class="tab-content">
				<div id="home" class="tab-pane fade in active">
					<div class="form-group col-md-12"  style="margin-top:30px">
						...
						{foreach $cates_parent_id_prod as $k}
							<option value="{$k->c_id}" {if ({$hienthithongtin->c_id}=={$k->c_id})}selected{/if}>{$k->c_name}</option>
						{/foreach}
						</select>
					</div>
					<div class="form-group col-md-12">
						...
						<input type="text" class="form-control" id="name" name="p_name" value="{$hienthithongtin->p_name}">
					</div>
					<div class="form-group col-md-9">
                    	<p style="color:red" id="err_price"></p>
						<label for="" class="col-md-2">Giá sản phẩm:</label>
						<div class="input-group">
							<input type="text" class="form-control" id="price" name="p_price" value="{$hienthithongtin->p_price}">
					...
					<div class="form-group col-md-12 form-inline">
						...
						<input id="p_image" class="form-control" name="p_image" type="text" value="/doan/public/images/san_pham/{$hienthithongtin->p_image}" size="60" onclick="BrowseServer( 'Images:/', 'xFilePath' );"/>
						...
						<img src="/doan/public/images/san_pham/{$hienthithongtin->p_image}" alt="" width="100"></div>
					</div>
					...
					<div class="form-group col-md-6">
						...
						<div class="input-group">
							<input type="text" class="form-control" readonly="readonly" name="p_sale" id="p_sale" value="{$hienthithongtin->p_sale}">
							<div class="input-group-addon">vnđ</div>
						</div>
					</div>
					<div class="form-group col-md-6">
			...
			<div id="menu1" class="tab-pane fade">
					<div class="form-group col-md-12 form-inline" style="margin-top:30px">
						...
						<input type="text" class="form-control" name="p_page" id="from" value="{$hienthithongtin->p_page}">
					</div>
					<div class="form-group col-md-12 form-inline">
						<label for="" class="col-md-2">Tác giả:</label>
						<select class="form-control" name="a_id" id="a_id">
							{foreach $author_cate as $a}
								<option value="{$a->a_id}" {if ({$hienthithongtin->a_id}=={$a->a_id})}selected{/if}>{$a->a_name}</option>
							{/foreach}
						</select>
					</div>
					<div class="form-group col-md-12 form-inline">
						...
						<select class="form-control" name="pl_id" id="pl_id">
							{foreach $publisher_cate as $p}
								<option value="{$p->pl_id}" {if ({$hienthithongtin->pl_id}=={$p->pl_id})}selected{/if}>{$p->pl_name}</option>
							{/foreach}
						</select>
					</div>
					<div class="form-group col-md-10 col-md-push-1 col-md-pull-1">
						<label for="">Mô tả sản phẩm:</label>
						<textarea name="p_describe" id="p_describe" class="form-control ckeditor" rows="10" style="resize:none;">{$hienthithongtin->p_describe}</textarea>
					</div>
			</div>
...
<div id="menu2" class="tab-pane fade">
		<div class="form-group col-md-12 form-inline" style="margin-top:30px">
			...
			<input id="p_thumbs_img1" class="form-control" name="p_thumbs_img1" value="/doan/public/images/thumbs/{$mang[0]}" type="text" size="60" onclick="BrowseServer( 'Images:/', 'xFilePath' );"/>
			<input type="button" class="btn btn-primary" value="Tải ảnh lên" onclick="BrowseServer( 'Images:/', 'p_thumbs_img1' );" />
		</div>
		<div class="form-group col-md-12 form-inline" style="margin-top:30px">
			<label for="" class="col-md-2">Hình ảnh:</label>
			
			<input id="p_thumbs_img2" class="form-control" name="p_thumbs_img2" value="/doan/public/images/thumbs/{$mang[1]}" type="text" size="60" onclick="BrowseServer( 'Images:/', 'xFilePath' );"/>
			<input type="button" class="btn btn-primary" value="Tải ảnh lên" onclick="BrowseServer( 'Images:/', 'p_thumbs_img2' );" />
		</div>
		<div class="form-group col-md-12 form-inline" style="margin-top:30px">
			<label for="" class="col-md-2">Hình ảnh:</label>
			
			<input id="p_thumbs_img3" class="form-control" name="p_thumbs_img3" value="/doan/public/images/thumbs/{$mang[2]}" type="text" size="60" onclick="BrowseServer( 'Images:/', 'xFilePath' );"/>
			<input type="button" class="btn btn-primary" value="Tải ảnh lên" onclick="BrowseServer( 'Images:/', 'p_thumbs_img3' );" />
		</div>
		<div class="col-md-offset-2">
		{foreach $mang as $row}
			<img src="/doan/public/images/thumbs/{$row}" alt="" width="200">
		{/foreach}
		</div>
	</form>
</div>
